...
class Generator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        # self.W1_deconv_mu = nn.Parameter(xavier_init((128, 64, 4, 4)).type(torch.FloatTensor),
        #                   requires_grad=True)
        # self.W1_deconv_rho = nn.Parameter(xavier_init((128, 64, 4, 4)).type(
        #     torch.FloatTensor), requires_grad=True)
        # self.W2_deconv_mu = nn.Parameter(xavier_init((64, 1, 4, 4)).type(torch.FloatTensor),
        #                                requires_grad=True)
        # self.W2_deconv_rho = nn.Parameter(xavier_init((64, 1, 4, 4)).type(torch.FloatTensor),
        #                                requires_grad=True)
    ...
    def compute_parameters(self):
        ...
        print("Mean deconv W1 {0}, var {1}".format(torch.mean(self.deconv1.weight.data), torch.std(
            self.deconv1.weight.data)))        # self.W3_sigma = torch.log(1 + torch.exp(
        print("Mean deconv W2 {0}, var {1}".format(torch.mean(self.deconv2.weight.data), torch.std(
    ...
    def forward(self, input=None):
        ...
        if input is not None:
            ...
            output_deconv_1 = f.leaky_relu(self.deconv1(x))
            output_deconv_1 = self.batch_norm3(output_deconv_1)
            output_deconv_2 = self.deconv2(output_deconv_1)
