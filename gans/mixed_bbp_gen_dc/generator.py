...
class Generator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        self.W1_deconv_mu = nn.Parameter(xavier_init((128, 64, 6, 6), convolution=True).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_deconv_rho = nn.Parameter(xavier_init((128, 64, 6, 6), convolution=True).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_deconv_mu = nn.Parameter(xavier_init((64, 3, 4, 4), convolution=True).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_deconv_rho = nn.Parameter(xavier_init((64, 3, 4, 4), convolution=True).type(
            torch.FloatTensor),
                                       requires_grad=True)
