...
from gans.utils import xavier_init, glorot_normal
...
class Generator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        self.num_samples = 1
        # layers
        self.W1_mu = nn.Parameter(glorot_normal((noise_dim, self.hidden_dims[0])).type(torch.FloatTensor),
                                  requires_grad=True)
        self.W1_rho = nn.Parameter(glorot_normal((noise_dim, self.hidden_dims[0])).type(torch.FloatTensor),
                                   requires_grad=True)
        self.W2_mu = nn.Parameter(glorot_normal((self.hidden_dims[0], 128*7*7)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_rho = nn.Parameter(glorot_normal((self.hidden_dims[0], 128*7*7)).type(
            torch.FloatTensor),
                                   requires_grad=True)
        ...
        #                            requires_grad=True)
        # # deconv layers
        self.W1_deconv_mu = self.std_mu * nn.Parameter(glorot_normal((128, 64, 6, 6)).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_deconv_rho = nn.Parameter(glorot_normal((128, 64, 6, 6)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_deconv_mu = self.std_mu * nn.Parameter(glorot_normal((64, 1, 4, 4)).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_deconv_rho = nn.Parameter(glorot_normal((64, 3, 4, 4)).type(
            torch.FloatTensor),
                                       requires_grad=True)


        # norm layers
        self.norm_layer1 = nn.BatchNorm1d(self.hidden_dims[0])
        self.norm_layer2 = nn.BatchNorm1d(128*7*7)
        self.norm_layer3 = nn.BatchNorm2d(64)

    def compute_parameters(self):
        # print('w1_mu', self.W1_mu)
        # print('w1_rho', self.W1_rho)
        self.W1_sigma = self.weight_std * torch.log(1 + torch.exp(self.W1_rho))
        self.W1 = (self.W1_mu + self.W1_sigma * \
                   Variable(torch.randn(self.W1_mu.size()), requires_grad=False))

        self.W2_sigma = self.weight_std * torch.log(1 + torch.exp(self.W2_rho))
        self.W2 = (self.W2_mu + self.W2_sigma * \
                   Variable(torch.randn(self.W2_mu.size()), requires_grad=False))
        ...
        self.W1_deconv_sigma = self.weight_std_conv * torch.log(1 + torch.exp(self.W1_deconv_rho))
        self.W1_deconv = (self.W1_deconv_mu + self.W1_deconv_sigma *
                        Variable(torch.randn(self.W1_deconv_sigma.size()),
                                 requires_grad=False))
        # print("Mean deconv W1 {0}, var {1}".format(np.mean(self.W1_deconv.data.numpy()), np.std(
        #     self.W1_deconv.data.numpy())))
        self.W2_deconv_sigma = self.weight_std_conv * torch.log(1 + torch.exp(self.W2_deconv_rho))
        self.W2_deconv = (self.W2_deconv_mu + self.W2_deconv_sigma *
                        Variable(torch.randn(self.W2_deconv_sigma.size()),
                                 requires_grad=False))
        # print("Mean deconv W2 {0}, var {1}".format(np.mean(self.W2_deconv.data.numpy()), np.std(
        #     self.W2_deconv.data.numpy())))

    def forward(self, input=None):
        self.compute_parameters()
        if input is not None:
            h1 = f.leaky_relu(torch.matmul(input, self.W1))
            h1 = self.norm_layer1(h1)
            h2 = f.leaky_relu(torch.matmul(h1, self.W2))
            h2 = self.norm_layer2(h2)
            ...
            output_deconv_1 = self.norm_layer3(output_deconv_1)
    ...
    def loss(self, scores_fake):
        ...
        target = 1
        log_pw, log_likelyhood_gauss, log_qw = 0, 0, 0
