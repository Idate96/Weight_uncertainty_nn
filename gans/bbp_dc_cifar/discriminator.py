...
class Discriminator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        # convlutional layers
        self.W1_conv_mu = self.std_mu*nn.Parameter(glorot_normal((32, 3, 5, 5)).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_conv_rho = nn.Parameter(glorot_normal((32, 3, 5, 5)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_conv_mu = self.std_mu*nn.Parameter(glorot_normal((32, 32, 5, 5)).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_conv_rho = nn.Parameter(glorot_normal((32, 32, 5, 5)).type(torch.FloatTensor),
                                       requires_grad=True)

        # linear layers
        self.W1_mu = nn.Parameter(glorot_normal((self.hidden_dims[0], self.hidden_dims[1])).type(
            torch.FloatTensor), requires_grad=True)
        self.W1_rho = nn.Parameter(glorot_normal((self.hidden_dims[0], self.hidden_dims[1])).type(
            torch.FloatTensor), requires_grad=True)
        ...
        self.W3_mu = nn.Parameter(glorot_normal((self.hidden_dims[1], 1)).type(torch.FloatTensor),
                                  requires_grad=True)
        self.W3_rho = nn.Parameter(glorot_normal((self.hidden_dims[1], 1)).type(torch.FloatTensor),
                                   requires_grad=True)
    ...
    def compute_parameters(self):
        ...
        self.W1_conv_sigma = self.weight_std_conv * torch.log(1 + torch.exp(self.W1_conv_rho))
        self.W1_conv = (self.W1_conv_mu + self.W1_conv_sigma *
                        Variable(torch.randn(self.W1_conv_sigma.size()),
                                 requires_grad=False))
        # print("Mean conv W1 {0}, var {1}" .format(np.mean(self.W1_conv.data.numpy()), np.std(
        #     self.W1_conv.data.numpy())))
        self.W2_conv_sigma = self.weight_std_conv * torch.log(1 + torch.exp(self.W2_conv_rho))
        self.W2_conv = (self.W2_conv_mu + self.W2_conv_sigma *
                        Variable(torch.randn(self.W2_conv_sigma.size()),
                                 requires_grad=False))
        # print("Mean conv W2 {0}, var {1}" .format(np.mean(self.W2_conv.data.numpy()), np.std(
        #     self.W2_conv.data.numpy())))


    def forward(self, input=None):
        self.compute_parameters()
        if input is not None:
            input = input.view(batch_size, 3, 32, 32)
            ...
            output_max_pool_2 = f.max_pool2d(output_conv_2, (2, 2))
