...
class Generator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        self.W1_sigma = nn.Parameter(self.sigma_factor * torch.abs(xavier_init((noise_dim,
                                                                              self.hidden_dims[
            0]))).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_mu = nn.Parameter(xavier_init((self.hidden_dims[0], 128*7*7)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_sigma = nn.Parameter(self.sigma_factor * torch.abs(xavier_init((self.hidden_dims[
                                                                                    0], 128*7*7))).type(
            torch.FloatTensor), requires_grad=True)
        ...
        #                            requires_grad=True)
        # # deconv layers
        self.W1_deconv_mu = nn.Parameter(xavier_init((128, 64, 6, 6), convolution=True).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_deconv_sigma = nn.Parameter(self.sigma_factor *torch.abs(xavier_init((128, 64, 6, 6),
                                                             convolution=True)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_deconv_mu = nn.Parameter(xavier_init((64, 3, 4, 4), convolution=True).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_deconv_sigma = nn.Parameter(self.sigma_factor *torch.abs(xavier_init((64, 3, 4, 4),
                                                               convolution=True)).type(
            torch.FloatTensor),
                                       requires_grad=True)
