...
class Generator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        #                            requires_grad=True)
        # # deconv layers
        self.W1_deconv_mu = self.std_mu * nn.Parameter(glorot_normal((128, 64, 4, 4)).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_deconv_rho = nn.Parameter(glorot_normal((128, 64, 4, 4)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_deconv_mu = self.std_mu * nn.Parameter(glorot_normal((64, 1, 4, 4)).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_deconv_rho = nn.Parameter(glorot_normal((64, 1, 4, 4)).type(
            torch.FloatTensor),
                                       requires_grad=True)
