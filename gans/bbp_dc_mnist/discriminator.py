...
class Discriminator(nn.Module):
    def __init__(self, learning_rate, hidden_dims, label=''):
        ...
        # convlutional layers
        self.W1_conv_mu = self.std_mu*nn.Parameter(glorot_normal((32, 1, 5, 5)).type(
            torch.FloatTensor),
                          requires_grad=True)
        self.W1_conv_rho = nn.Parameter(glorot_normal((32, 1, 5, 5)).type(
            torch.FloatTensor), requires_grad=True)
        self.W2_conv_mu = self.std_mu*nn.Parameter(glorot_normal((64, 32, 5, 5)).type(
            torch.FloatTensor),
                                       requires_grad=True)
        self.W2_conv_rho = nn.Parameter(glorot_normal((64, 32, 5, 5)).type(torch.FloatTensor),
                                       requires_grad=True)
